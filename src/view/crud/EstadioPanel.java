...
import controller.EstadioController;
...
/**
 * EstadioPanel - CRUD de estadios.
 */
public class EstadioPanel extends JPanel {
    ...
    private JPanel buildHeader() {
        ...
        p.setBackground(BG);
        JLabel t = new JLabel("Estadios");
        t.setFont(new Font("Segoe UI", Font.BOLD, 20));
        t.setForeground(TEXT_PRI);
        JLabel s = new JLabel("Estadios sede de la fase de grupos");
    ...
    private JScrollPane buildTabla() {
        modelo = new DefaultTableModel(
                new String[]{"ID", "Nombre", "Ciudad", "Capacidad"}, 0) {
    ...
    private JPanel buildFormulario() {
        ...
        JLabel titulo = new JLabel("Datos del estadio");
        titulo.setFont(new Font("Segoe UI", Font.BOLD, 15));
        titulo.setForeground(TEXT_PRI);
        titulo.setAlignmentX(LEFT_ALIGNMENT);

        txtNombre    = makeTextField();
        txtCapacidad = makeTextField();

        cmbCiudad = new JComboBox<>();
        cmbCiudad.setFont(new Font("Segoe UI", Font.PLAIN, 13));
        cmbCiudad.setMaximumSize(new Dimension(Integer.MAX_VALUE, 38));
        cmbCiudad.setAlignmentX(LEFT_ALIGNMENT);
        for (Ciudad c : ciudadDAO.listarTodos()) cmbCiudad.addItem(c);
        ...
        form.add(makeLabel("Nombre del estadio"));
        form.add(Box.createVerticalStrut(6));
        form.add(txtNombre);
        form.add(Box.createVerticalStrut(14));
        form.add(makeLabel("Ciudad"));
        form.add(Box.createVerticalStrut(6));
        form.add(cmbCiudad);
        form.add(Box.createVerticalStrut(14));
        form.add(makeLabel("Capacidad"));
        form.add(Box.createVerticalStrut(6));
    ...
    private void guardar() {
        ...
        if (ciudad == null)   { mostrarMensaje("Selecciona una ciudad.", true); return; }
        int cap = 0;
        if (!capStr.isEmpty()) {
            try { cap = Integer.parseInt(capStr); }
            catch (NumberFormatException e) { mostrarMensaje("La capacidad debe ser un número.", true); return; }
        }
        String resultado;
        if (idSeleccionado == -1) {
            resultado = controller.insertar(new Estadio(nombre, cap, ciudad.getIdCiudad()));
        } else {
            resultado = controller.actualizar(new Estadio(idSeleccionado, nombre, cap, ciudad.getIdCiudad()));
        }
        boolean esError = resultado.startsWith("Error") || resultado.startsWith("Sin");
        mostrarMensaje(resultado, esError);
        if (esError) {
            JOptionPane.showMessageDialog(this, resultado, "No se pudo guardar el estadio", JOptionPane.WARNING_MESSAGE);
            return;
        }
        cargarTabla();
        limpiarFormulario();
    ...
    private void eliminar() {
        ...
        int c = JOptionPane.showConfirmDialog(this, "¿Eliminar este estadio?", "Confirmar", JOptionPane.YES_NO_OPTION);
        ...
        if (esErrorElim) {
            JOptionPane.showMessageDialog(this, resultado, "No se pudo eliminar el estadio", JOptionPane.WARNING_MESSAGE);
            return;
        }
        cargarTabla();
        limpiarFormulario();
    ...
    private void cargarTabla() {
        ...
        for (Estadio e : controller.listarTodos())
            modelo.addRow(new Object[]{e.getIdEstadio(), e.getNombre(), e.getNombreCiudad(), e.getCapacidad()});
    }
    ...
    private void cargarEnFormulario() {
        ...
        Estadio e = controller.buscarPorId(idSeleccionado);
        if (e == null) return;
        txtNombre.setText(e.getNombre());
        txtCapacidad.setText(String.valueOf(e.getCapacidad()));
        for (int i = 0; i < cmbCiudad.getItemCount(); i++) {
            if (cmbCiudad.getItemAt(i).getIdCiudad() == e.getIdCiudad()) {
                cmbCiudad.setSelectedIndex(i); break;
            }
        }
    ...
    private void limpiarFormulario() {
        ...
        if (cmbCiudad.getItemCount() > 0) cmbCiudad.setSelectedIndex(0);
        btnEliminar.setEnabled(false);
        lblMensaje.setText(" ");
