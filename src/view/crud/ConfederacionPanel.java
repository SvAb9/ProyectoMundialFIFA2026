...
import controller.ConfederacionController;
...
/**
 * ConfederacionPanel - CRUD de confederaciones.
 */
public class ConfederacionPanel extends JPanel {
    ...
    private final ConfederacionController controller = new ConfederacionController();
    ...
    private JPanel buildHeader() {
        ...
        p.setBackground(BG);
        JLabel titulo = new JLabel("Confederaciones");
        titulo.setFont(new Font("Segoe UI", Font.BOLD, 20));
        titulo.setForeground(TEXT_PRI);
        JLabel sub = new JLabel("Gestión de confederaciones");
        sub.setFont(new Font("Segoe UI", Font.PLAIN, 13));
        sub.setForeground(TEXT_SEC);
        JPanel t = new JPanel();
        t.setLayout(new BoxLayout(t, BoxLayout.Y_AXIS));
        t.setBackground(BG);
        t.add(titulo); t.add(sub);
    ...
    private JPanel buildFormulario() {
        ...
        JLabel titulo = new JLabel("Datos de la confederación");
    ...
    private void guardar() {
        ...
        String resultado;
        if (idSeleccionado == -1) {
            resultado = controller.insertar(new Confederacion(nombre));
        } else {
    ...
    private void cargarTabla() {
        ...
        for (Confederacion c : controller.listarTodos())
            modelo.addRow(new Object[]{c.getIdConfederacion(), c.getNombre()});
    }
    ...
    private void cargarEnFormulario() {
        ...
        Confederacion c = controller.buscarPorId(idSeleccionado);
        if (c == null) return;
