...
import controller.DirectorTecnicoController;
...
/**
 * DirectorTecnicoPanel - CRUD de directores técnicos.
 */
public class DirectorTecnicoPanel extends JPanel {
    ...
    private JTextField          txtNombre, txtApellido, txtNacionalidad;
    private JComboBox<Equipo>   cmbEquipo;
    private JLabel              lblMensaje;
    private JButton             btnGuardar, btnEliminar, btnNuevo;

    private final DirectorTecnicoController controller = new DirectorTecnicoController();
    private final EquipoDAO                 equipoDAO  = new EquipoDAO();
    private int idSeleccionado = -1;

    public DirectorTecnicoPanel() {
    ...
    private JPanel buildHeader() {
        ...
        p.setBackground(BG);
        JLabel t = new JLabel("Directores Técnicos");
        t.setFont(new Font("Segoe UI", Font.BOLD, 20));
        t.setForeground(TEXT_PRI);
        JLabel s = new JLabel("Entrenadores de cada selección");
    ...
    private JScrollPane buildTabla() {
        modelo = new DefaultTableModel(
                new String[]{"ID", "Nombre", "Apellido", "Nacionalidad", "Equipo"}, 0) {
    ...
    private JPanel buildFormulario() {
        ...
        JLabel titulo = new JLabel("Datos del director técnico");
        ...
        form.add(makeLabel("Apellido"));
        form.add(Box.createVerticalStrut(6));
        form.add(txtApellido);
        form.add(Box.createVerticalStrut(14));
        form.add(makeLabel("Nacionalidad"));
        form.add(Box.createVerticalStrut(6));
        form.add(txtNacionalidad);
        form.add(Box.createVerticalStrut(14));
        form.add(makeLabel("Equipo"));
        form.add(Box.createVerticalStrut(6));
        form.add(cmbEquipo);
    ...
    private void guardar() {
        ...
        String resultado;
        if (idSeleccionado == -1) {
            resultado = controller.insertar(new DirectorTecnico(nombre, apellido, nacionalidad, equipo.getIdEquipo()));
        } else {
    ...
    private void cargarTabla() {
        ...
        for (DirectorTecnico dt : controller.listarTodos())
            modelo.addRow(new Object[]{dt.getIdDirector(), dt.getNombre(), dt.getApellido(), dt.getNacionalidad(), dt.getNombreEquipo()});
    }
    ...
    private void cargarEnFormulario() {
        ...
        DirectorTecnico dt = controller.buscarPorId(idSeleccionado);
        if (dt == null) return;
        txtNombre.setText(dt.getNombre());
        txtApellido.setText(dt.getApellido());
        txtNacionalidad.setText(dt.getNacionalidad());
        for (int i = 0; i < cmbEquipo.getItemCount(); i++) {
            if (cmbEquipo.getItemAt(i).getIdEquipo() == dt.getIdEquipo()) {
    ...
    private void limpiarFormulario() {
        idSeleccionado = -1;
        txtNombre.setText(""); txtApellido.setText(""); txtNacionalidad.setText("");
